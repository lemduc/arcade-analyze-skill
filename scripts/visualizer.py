...
function renderKnow(){
  const cards=DATA.metrics.map(m=>{
    ...
    // TurboMQ's value is an unbounded sum of cluster factors; judge the normalized form.
    const hv=(m.name==='TurboMQ'&&m.details&&m.details.normalized!=null)?+m.details.normalized:+m.value;
    return `<div class="kcard"><div class="kname">${esc(m.name)}</div><div class="kval">${(+m.value).toFixed(3)}</div>`+
      `<p>${esc(metricHelp(m.name,hv))}</p>`+(extra?`<p class="kmeta">${esc(extra)}</p>`:'')+'</div>';}).join('');
